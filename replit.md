...
- Glimr subscribe endpoints (`/api/companion/subscribe/{checkout,verify,status,portal}`): identity is session-first — `status`/`portal` derive the subscriber from `req.user` (401 if not signed in) and ignore any client-supplied `email`; `checkout` accepts an optional `email` only as an anonymous-checkout prefill; `verify` resolves email from the completed Checkout session
...
- Glimr has real Replit Auth sign-in (`/login`, `/account`) built on `@workspace/replit-auth-web`; browsing/chat stay anonymous-friendly (no app-wide auth gate), only `/account` and the billing identity calls (`/companion/subscribe/status`, `/portal`, plus persona-create and video-call in `companion.ts`) require or prefer a session — this closed the earlier client-supplied-email takeover gap on those routes
