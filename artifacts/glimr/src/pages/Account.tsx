...
function loadProfile(userId: string | null): AccountProfile {
  if (!userId) return defaultProfile;
  try {
    const raw = localStorage.getItem(`glimr_account_profile_${userId}`);
...
export default function Account() {
  ...
  const [profile, setProfile] = useState<AccountProfile>(() => loadProfile(user?.id ?? null));
  ...
  const handleOpenPortal = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "We couldn't find a billing account yet. Subscribe to a plan first, then try again.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleSave = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (user?.id) localStorage.setItem(`glimr_account_profile_${user.id}`, JSON.stringify(profile));
    setSaved(true);
  ...
  if (authLoading) {
    return (
      <div className="min-h-[100dvh] bg-background text-foreground">
        <SiteNav />
        <main className="mx-auto flex min-h-[70dvh] w-full max-w-3xl items-center justify-center px-5">
              ...
              {[
                ...
                <label key={key} className="block space-y-2">
                  ...
                  <input
                    ...
                    onChange={(event) => setProfile({ ...profile, [key as keyof AccountProfile]: event.target.value })}
