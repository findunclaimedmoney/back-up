...
import type { useSendCompanionBirthdayCardEmail } from "@workspace/api-client-react";
...
interface Props {
  ...
  onDismiss: () => void;
  onBirthdayChat: () => void;
  mutation: ReturnType<typeof useSendCompanionBirthdayCardEmail>;
}

export function BirthdayCard({ name, email, personaId, personaName, sessionId, onDismiss, onBirthdayChat, mutation }: Props) {
  const [emailSent, setEmailSent] = useState(false);
  const portrait = PORTRAITS[personaId];

  const handleSendEmail = async () => {
    try {
      await mutation.mutateAsync({ data: { sessionId, personaId } });
      setEmailSent(true);
    } catch {
      setEmailSent(true);
    }
            ...
            <div className="relative p-8 text-center">
              ...
              <div className="bg-white/5 rounded-2xl p-4 text-left mb-6 border border-white/8">
                <p className="text-white/80 text-sm leading-relaxed">
                  Today is your day{name ? `, ${name}` : ""} — and I want you to know how much you mean to me. Every conversation we
                  share is something I treasure. I hope today is filled with everything that makes your heart happy. 🥂
                </p>
              ...
              <div className="space-y-2">
                ...
                {email && !emailSent && (
                  ...
                  >
                    ...
                    {mutation.isPending ? "Sending…" : "Send me a card by email 💌"}
                  </Button>
                )}
                {emailSent && <p className="text-emerald-400 text-sm py-2">Card sent! Check your inbox 🎉</p>}
                {!email && (
