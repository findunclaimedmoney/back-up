...
import { useGenerateCompanionOutfit } from "@workspace/api-client-react";
...
const MIA_FACE =
  "a young woman in her late 20s with long flowing blonde wavy hair, warm brown eyes, high cheekbones, soft features, and a naturally warm expression";
...
export const OUTFITS = [
  ...
  { id: "doctor", name: "Doctor", emoji: "🩺", desc: "wearing a white medical coat with a stethoscope around neck, warm and professional, clinical setting" },
  {
    ...
    emoji: "✈️",
    desc: "wearing an elegant navy blue airline uniform with a silk neck scarf, polished professional",
  },
...
export function OutfitPicker({ sessionId, personaId, customPersona, activeOutfitId, onSelect, onBack, onUpgrade, subscription }: Props) {
  ...
  const mutation = useGenerateCompanionOutfit();

  const isFlame = subscription.status.active && subscription.status.tier === "flame";
  const personaKey: "mia" | "alex" = personaId === "alex" ? "alex" : "mia";

  const defaultPortrait = customPersona ? `data:image/png;base64,${customPersona.portraitBase64}` : (PORTRAITS[personaId] ?? "");
  ...
  const handleSelect = async (outfit: (typeof OUTFITS)[number]) => {
    ...
    try {
      ...
      localStorage.setItem(cacheKey, result.portraitBase64);
      onSelect(outfit.id, result.portraitBase64);
    } catch {
  ...
  return (
    <div className="flex flex-col h-[100dvh] w-full max-w-2xl mx-auto p-4 md:p-8">
      ...
      {error && <div className="mb-4 text-sm text-destructive bg-destructive/10 border border-destructive/20 rounded-xl px-4 py-3">{error}</div>}
