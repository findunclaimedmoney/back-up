...
import {
  ...
  CreateCompanionPersonaResponse,
  CompanionChatBody,
  CompanionChatResponse,
  GetCompanionMemoryResponse,
  SaveCompanionMemoryBody,
  SaveCompanionMemoryResponse,
  GetCompanionFactsResponse,
  GetCompanionBirthdayCheckResponse,
  SendCompanionBirthdayCardEmailBody,
  SendCompanionBirthdayCardEmailResponse,
  GenerateCompanionOutfitBody,
  GenerateCompanionOutfitResponse,
  CreateCompanionVideoBody,
  CreateCompanionVideoResponse,
...
- Keep it human. Real. Warm.`,
  },
  {
    ...
    description:
      "Alex is steady and reliable — the kind of presence that cuts through noise and helps you think clearly. Honest without being harsh, supportive without being soft.",
...
router.get("/companion/personas", (_req, res): void => {
  res.json(
    GetPersonasResponse.parse(
      PERSONAS.map(({ id, name, gender, tagline, description }) => ({ id, name, gender, tagline, description })),
    ),
  );
});

router.post("/companion/persona/create", async (req, res): Promise<void> => {
  const parsed = CreateCompanionPersonaBody.safeParse(req.body);
  if (!parsed.success) {
  ...
  try {
    ...
    const jsonParsed = jsonMatch ? (JSON.parse(jsonMatch[0]) as { description?: string; name?: string }) : {};
    const faceDescription = jsonParsed.description ?? "A friendly, approachable person";
    const suggestedName = jsonParsed.name ?? "Jamie";
    ...
    res.json(CreateCompanionPersonaResponse.parse({ portraitBase64, faceDescription, suggestedName }));
  } catch (err) {
    req.log.error({ err }, "persona create error");
    res.status(500).json({ error: "Failed to generate companion" });
  }
});

router.post("/companion/chat", async (req, res): Promise<void> => {
  const parsed = CompanionChatBody.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: parsed.error.message });
    return;
  }
  ...
  try {
    ...
    const systemPrompt =
      contextParts.length > 0 ? persona.systemPrompt + "\n\n---" + contextParts.join("") : persona.systemPrompt;
    ...
    res.json(CompanionChatResponse.parse({ responseText, sessionId, audioBase64 }));
  } catch (err) {
    req.log.error({ err }, "companion chat error");
    res.status(500).json({ error: "Something went wrong" });
  }
});

router.get("/companion/memory/:sessionId", async (req, res): Promise<void> => {
  ...
  res.json(
    GetCompanionMemoryResponse.parse({
      sessionId,
...
router.post("/companion/memory", async (req, res): Promise<void> => {
  const parsed = SaveCompanionMemoryBody.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: parsed.error.message });
    return;
  }
  const { sessionId } = parsed.data;
  const personaId = parsed.data.persona ?? "mia";
  ...
  try {
    ...
    if (openaiKey && messages.length > 2) {
      ...
      const transcript = messages.map((m) => `${m.role === "user" ? "User" : "Companion"}: ${m.content}`).join("\n");
...
Return {} if nothing new. Only include keys for information explicitly stated in the conversation.`,
      ...
      try {
        ...
        req.log.warn("Failed to parse extracted facts JSON");
      }
    }
    ...
    res.json(
      SaveCompanionMemoryResponse.parse({ sessionId, summary, messageCount: messages.length, updatedAt: new Date() }),
    );
  } catch (err) {
    req.log.error({ err }, "save memory error");
    res.status(500).json({ error: "Failed to save memory" });
  }
});

router.get("/companion/facts/:sessionId", async (req, res): Promise<void> => {
  const sessionId = Array.isArray(req.params.sessionId) ? req.params.sessionId[0]! : req.params.sessionId;
  const facts = await db.select().from(companionFactsTable).where(eq(companionFactsTable.sessionId, sessionId));
  const factMap = Object.fromEntries(facts.map((f) => [f.factKey, f.factValue]));
  res.json(GetCompanionFactsResponse.parse({ sessionId, facts: factMap }));
});

router.get("/companion/birthday-check/:sessionId", async (req, res): Promise<void> => {
  const sessionId = Array.isArray(req.params.sessionId) ? req.params.sessionId[0]! : req.params.sessionId;
  ...
  res.json(
    GetCompanionBirthdayCheckResponse.parse({
      isBirthday,
...
router.post("/companion/birthday-card-email", async (req, res): Promise<void> => {
  const parsed = SendCompanionBirthdayCardEmailBody.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: parsed.error.message });
    return;
  }
  ...
  try {
    ...
    res.json(SendCompanionBirthdayCardEmailResponse.parse({ sent: true, to: recipientEmail }));
  } catch (err) {
    req.log.error({ err }, "birthday card email failed");
    res.status(500).json({ error: "Failed to send email" });
  }
});

router.post("/companion/outfit/generate", async (req, res): Promise<void> => {
  const parsed = GenerateCompanionOutfitBody.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: parsed.error.message });
    return;
  }
  const { sessionId, outfitId, outfitDescription } = parsed.data;
  const faceDescription = parsed.data.faceDescription ?? "an attractive young person";
  ...
  try {
    ...
    if (match) {
      res.json(GenerateCompanionOutfitResponse.parse({ portraitBase64: match.portraitBase64, cached: true }));
      return;
    }
    ...
    res.json(GenerateCompanionOutfitResponse.parse({ portraitBase64, cached: false }));
  } catch (err) {
    req.log.error({ err }, "outfit generate error");
    res.status(500).json({ error: "Failed to generate outfit" });
  }
});

router.post("/companion/video", async (req, res): Promise<void> => {
  const parsed = CreateCompanionVideoBody.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: parsed.error.message });
    return;
  }
  ...
  try {
    ...
    if (!createRes.ok) {
      ...
      req.log.error({ errText }, "HeyGen create video failed");
    ...
    res.json(CreateCompanionVideoResponse.parse({ videoUrl }));
  } catch (err) {
    req.log.error({ err }, "HeyGen video error");
